...
const CreatePost = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.prompt && form.photo) {
      ...
      try {
        await addPost()    
        alert('Success, your post will be loaded shortly');
        navigate('/');
  ...
  return (
    <section className="sm:w-screen p-4">
      <div>
        <h1 className="font-extrabold text-white text-[32px]">Add Post</h1>
      ...
      <form className="mt-16 max-w-3xl" onSubmit={handleSubmit}>
        ...
        <div className="mt-10">
          <p className="mt-2 text-white text-[14px]">** Once you have created the image you want, 
          you can post it in the gallery **</p>
          <button
            type="submit"
            className="mt-3 text-white bg-[#6469ff] font-medium rounded-md text-sm w-full sm:w-auto 
            px-5 py-2.5 text-center"
          >
            {loading ? 'Sharing...' : 'Post in the Gallery'}
          </button>
        </div>
